module DomainHelper
  ...
  def make_image
    ...
    if domain_suff.include? 'bike' then content_tag(cl_image_tag("/assets/VT750.png", :width => 4000, :height => 5500, :crop => :fill))  end
    if domain_suff.include? 'honda' then content_tag(cl_image_tag("/assets/honda_cbr_600rr_black_wide.jpg", :width => 4000, :height => 5500, :crop => :fill))  end
